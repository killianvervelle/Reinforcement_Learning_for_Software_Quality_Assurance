...
public class JFuzzyLogic {
	...
	String args[];
	net.sourceforge.jFuzzyLogic.FIS fis;
	...
	void compile(String fileName) {
		...
		if (debug) {
			String cfile = net.sourceforge.jFuzzyLogic.Gpr.HOME + "/x.cpp";
			net.sourceforge.jFuzzyLogic.Gpr.debug("Writing to file " + cfile);
			net.sourceforge.jFuzzyLogic.Gpr.toFile(cfile, fis.toStringCpp());
		}
	}
	...
	int evaluate(int argNum) {
		...
		for (net.sourceforge.jFuzzyLogic.FunctionBlock fb : fis) {
			for (Variable var : fb.variablesSorted()) {
				if (var.isInput()) {
					if (argNum < args.length) var.setValue(net.sourceforge.jFuzzyLogic.Gpr.parseDoubleSafe(args[argNum++]));
					else {
	...
	public net.sourceforge.jFuzzyLogic.FIS getFis() {
		return fis;
	}
	 ...
	 */
	net.sourceforge.jFuzzyLogic.FIS load(String fclFileName) {
		try {
