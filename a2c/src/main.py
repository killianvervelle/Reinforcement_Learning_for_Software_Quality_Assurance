...
class StressTesting:
    ...
    def main(self):
        ...
        vms = self.initialize_vms(10, 100, 100, 100, requirement_res_times, sensitivity_collection)
        ...
        actor_optim = optim.Adam(agent.actor.parameters(), lr=self.learning_rate)
        critic_optim = optim.Adam(agent.critic.parameters(), lr=self.learning_rate)
        ...
        for i in range(self.maximum_episodes):
            ...
            rewards, critic_vals, actor_probs, total_reward = agent.train_model(render=False)
            r.append(total_reward)

            loss_actor, loss_critic = agent.compute_loss(actor_probs=actor_probs, actual_returns=rewards, expected_returns=critic_vals)
            ...
            if i % 50 == 0 and i > 0:
                ...
                print(f"Average reward during episodes {i-50}-{i} is {average_reward}.")
    ...
    def initialize_vms(n, vms_cap_cpu, vms_cap_mem, vms_cap_disk, requirement_res_times, sensitivity_collection):
        ...
        for i in range(n):
            ...
            vm_sensitivity_index = random.randint(0, len(sensitivity_collection) - 1)
            vm_required_res_time_index = random.randint(0, len(requirement_res_times) - 1)
            ...
            print(f"Required Response Time: {requirement_res_times[vm_required_res_time_index]}")
